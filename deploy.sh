...
sam deploy --template-file template-built.yaml --stack-name ${app_name} --capabilities CAPABILITY_NAMED_IAM --no-fail-on-empty-changeset --parameter-overrides ServiceSubDomainName=freeze

rm template-built.yaml
cd vue && npm run build
aws s3 cp ./dist s3://freeze.coldlambda.com --recursive  